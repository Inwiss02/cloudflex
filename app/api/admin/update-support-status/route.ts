...
async function sendSupportWebhookNotification(supportRequest: any, oldStatus: string) {
  try {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: currentRequest } = await supabase.from("support_requests").select().eq("id", id).single()
